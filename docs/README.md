# GameMakerGraph Documentation

Status: current for 0.5.0

## Product and architecture

- [Product overview](product/overview.md)
- [Architecture overview](architecture/overview.md)
- [Continuous memory](architecture/continuous-memory.md)
- [Implementation map](architecture/implementation-map.md)
- [Product boundary](architecture/product-boundary.md)

## Contracts and specifications

- [MCP tools](contracts/mcp-tools.md)
- [CodeGraph provider](contracts/codegraph-provider.md)
- [Semantic model](specs/semantic-model.md)
- [GameGraph core](specs/gamegraph-core.md)
- [Documentation framework](specs/documentation-framework.md)

## Development and validation

- [Setup](development/setup.md)
- [Testing](development/testing.md)
- [Project-memory template](development/project-memory.md)
- [TapTap Maker loop](validation/taptap-maker-loop.md)

Repository files and confirmed controlled Markdown remain authoritative; indexes are disposable.
